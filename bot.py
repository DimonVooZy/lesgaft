import logging
import os
import threading
from flask import Flask
from telegram import Update, ReplyKeyboardMarkup
from telegram.ext import Application, CommandHandler, ContextTypes, MessageHandler, filters
import json

# Настройка логирования
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    level=logging.WARNING
)

# Отключаем логирование для httpx и httpcore
logging.getLogger("httpx").setLevel(logging.WARNING)
logging.getLogger("httpcore").setLevel(logging.WARNING)
logging.getLogger("telegram").setLevel(logging.WARNING)

# Наш логгер
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

# Токен бота из переменных окружения Render
BOT_TOKEN = os.environ.get('BOT_TOKEN', '')

# Файл для хранения статистики
STATS_FILE = "bot_stats.json"

# Загрузка статистики
def load_stats():
    try:
        if os.path.exists(STATS_FILE):
            with open(STATS_FILE, 'r', encoding='utf-8') as f:
                stats = json.load(f)
                # Миграция для изменения названия кнопки
                if "📚 История" in stats:
                    stats["📚 История кафедры"] = stats["📚 История"]
                    del stats["📚 История"]
                return stats
    except Exception as e:
        logger.error(f"Ошибка загрузки статистики: {e}")
    
    # Стандартная статистика при первом запуске
    return {
        "📢 Новости": 0,
        "🗓️ Расписание консультаций": 0,
        "📚 История кафедры": 0,
        "🎓 Абитуриентам": 0,
        "👨‍🎓 Студентам": 0,
        "⚽ Спортивная работа": 0,
        "🏅 Центр тестирования ГТО": 0,
        "👨‍🏫 Сотрудники кафедры": 0
    }

# Сохранение статистики
def save_stats(stats):
    try:
        with open(STATS_FILE, 'w', encoding='utf-8') as f:
            json.dump(stats, f, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.error(f"Ошибка сохранения статистики: {e}")

# Инициализация статистики
button_stats = load_stats()

# Тексты для разных разделов
WELCOME_TEXT = """
<b>Вы находитесь на официальном канале кафедры теории и методики массовой физкультурно-оздоровительной работы НГУ им. П.Ф. Лесгафта, Санкт-Петербург</b>
"""

NEWS_TEXT = """
<b>📢 Новости кафедры</b>

Здесь будут размещаться актуальные новости и события кафедры теории и методики массовой физкультурно-оздоровительной работы.

Следите за обновлениями!
"""

CONSULTATION_SCHEDULE_TEXT = """
<b>🗓️ Расписание консультаций</b>

Расписание консультаций преподавателей кафедры:

<u>Понедельник:</u> 10:00 - 12:00
<u>Вторник:</u> 14:00 - 16:00  
<u>Среда:</u> 10:00 - 12:00
<u>Четверг:</u> 14:00 - 16:00
<u>Пятница:</u> 10:00 - 12:00

Для уточнения расписания конкретного преподавателя обращайтесь по телефону кафедры.
"""

# Разбиваем историю на части
HISTORY_TEXT_PART1 = """
<b>📚 История кафедры</b>

На реализацию идей о том, что физкультура должна входить в повседневную жизнь населения, было направлено <b>Постановление ЦК КПСС и Совета Министров СССР «О дальнейшем подъеме массовости физической культуры и спорта»</b> (сентябрь 1981). Оно представляло собой развернутую программу физкультурного движения в новых условиях и охватывало важнейшие его проблемы.

Комитету по физической культуре и спорту при Совете Министров СССР и его органам на местах предлагалось «…концентрироваться на массовом развитии физической культуры, спорта, туризма, активного отдыха на реализации установок ХХVI съезда КПСС о внедрении физической культуры в быт каждой советской семьи…». В Постановлении говорилось о необходимости «…организовать подготовку специалистов по массовой физкультурно-оздоровительной работе и туризму.»

Массовая физкультурно-оздоровительная работа стала общегосударственным мероприятием и требовала для своего руководства новых энергичных молодых кадров, поэтому в стенах ведущего физкультурного ВУЗа страны – <b>Государственном дважды орденоносном институте физической культуры им. П.Ф. Лесгафта (ГДОИФК)</b> разрабатывается идея по созданию соответствующей кафедры с названием «Кафедра массовой физкультурно-оздоровительной работы и туризма (МФОРиТ)»

<u>🏛️ 1983 год</u> - кафедра МФОРиТ была создана. Новую кафедру было поручено возглавить энергичному специалисту в системе физического воспитания и спорта <b>к.п.н., доценту Вячеславу Георгиевичу Каневцу</b>.

Кафедра в начале своего становления была ориентирована на разработку новых направлений в массовой физической культуре, в создании соответствующей концепции по внедрению этих направлений в школах, ВУЗах, на предприятиях и среди населения страны.
"""

HISTORY_TEXT_PART2 = """
<u>👥 1983–1984 годы</u> - формирование коллектива кафедры грамотными и профессионально подготовленными педагогами:
• <b>Л.П. Трофимова</b> - к.п.н., доцент
• <b>А.В. Лебедев</b> - к.п.н., доцент  
• <b>Р.М. Орлов</b> - д.э.н., профессор
• <b>Д.Г. Незнамов</b>
• <b>Л.В. Жестяников</b>
• <b>А.А. Малышев</b>

<u>📅 С 1984 года</u> - кафедра проводит теоретические и методические конференции с приглашением отечественных и зарубежных специалистов.

Первые проблемы, которые решала кафедра в интересах массовости были ориентированы на развитие системы <b>ГТО</b>.

<u>🌟 Дальнейшее развитие коллектива:</u>
• <b>Г.А. Шашкин</b>
• <b>Ю.Н. Федотов</b>  
• <b>А.Н. Потапчук</b>
• <b>В.Г. Чванов</b>
• <b>Т.В. Платонова</b>
• <b>Т.В. Казанкина</b> - трехкратная олимпийская чемпионка

Первые годы кафедра выпускала преподавателей-организаторов физкультурно-оздоровительной работы и туризма.

На фоне массовой физкультурно-оздоровительной работы на кафедре открывается новое направление <b>«спортивно-оздоровительный туризм»</b> и обучение студентов идет по двум специализациям: физкультурно-оздоровительные технологии (ФОТ) и спортивно-оздоровительный туризм (СОТ).
"""

HISTORY_TEXT_PART3 = """
<u>🥇 1996–2000 годы</u> - кафедру возглавляла трехкратная олимпийская чемпионка <b>к.п.н., доцент Татьяна Васильевна Казанкина</b>.

<u>🎓 1999 год</u> - в России впервые в мировой практике начинается подготовка магистров по специализации «спортивно-оздоровительный туризм».

<u>🏔️ 2001–2011 годы</u> - кафедру возглавил <b>академик Российской академии туризма Юрий Николаевич Федотов</b>. Кафедра становится ведущей в России по подготовке специалистов по спортивно-оздоровительному туризму.

<u>🏛️ 2002 год</u> - создан региональный учебно-методический центр спортивно-оздоровительного туризма.

<u>🔬 2000–2010 годы</u> - научные достижения:
• Защищены 4 докторские диссертации
• Защищены 5 кандидатских диссертаций  
• Опубликовано более 500 научных работ

<u>🏆 Спортивные достижения студентов:</u>
• <b>М. Мамлеев</b> - мастер спорта международного класса по ориентированию
• <b>А. Левицкий</b> - чемпион мира по рафтингу (2004)
• <b>М. Ксенофонтов и А. Петрашов</b> - чемпионы России по туризму и Европы по рафтингу (2006)
"""

HISTORY_TEXT_PART4 = """
<u>🥌 2011–2015 годы</u> - кафедру возглавил <b>главный тренер сборной России по кёрлингу Ю.В. Шулико</b>. Появляется новый профиль обучения – «кёрлинг».

<u>👨‍🏫 С 2015 года</u> - кафедру возглавляет <b>к.п.н., доцент А.Б. Петров</b>. После реорганизации кафедра получает название <b>«Кафедра теории и методика массовой физкультурно-оздоровительной работы» (ТиММФОР)</b>.

<u>🏅 Награды преподавателей:</u>
• <b>Г.М. Лаврухина</b> - Отличник физической культуры и спорта, Ветеран труда
• <b>А.Б. Петров</b> - Отличник ФКиС, Грамота Президента РФ
• <b>Т.В. Горбунова</b> - Почётная грамота Комитета по ФКиС, Благодарность Губернатора
• <b>Ю.Ю. Вишнякова</b> - Благодарность Министра спорта, Почетная грамота Минспорта РФ

<u>👩‍💼 С марта 2025 года</u> - заведующий кафедрой <b>Ю.Ю. Вишнякова</b>.

<u>🎯 2025 год</u> - возобновил деятельность <b>центр тестирования ВФСК ГТО</b> под руководством <b>к.п.н., доцента Аксеновой Н.Н.</b>
"""

HISTORY_TEXT_PART5 = """
<u>💫 Современные достижения студентов:</u>
• Лауреаты Конкурса дипломных проектов
• Победители научно-практических конференций  
• Спортивные достижения в кёрлинге, полиатлоне, стрельбе из лука
• Победители всероссийских фестивалей и соревнований

Выпускники кафедры работают в образовательных организациях, Центрах физической культуры, спортивных клубах, фитнес-центрах, учреждениях дополнительного образования по всему Санкт-Петербургу.
"""

APPLICANTS_TEXT = """
<b>🎓 Абитуриентам</b>

<u>Направления подготовки:</u>
• Бакалавриат: 49.03.01 Физическая культура. Направленность (профиль) «Физкультурно-оздоровительная деятельность»
• Бакалавриат: 49.03.04 Спорт. Направленность (профиль) «Тренерско-преподавательская деятельность в избранном виде спорта» (полиатлон), с присвоением квалификации «Тренер по виду спорта. Преподаватель»
• Магистратура: 49.04.03 Спорт

Уважаемые абитуриенты! Вся актуальная информация на странице приемной комиссии НГУ им. П.Ф.Лесгафта, Санкт-Петербург
https://lesgaft.spb.ru/ru/commission/priyomnaya-komissiya
"""

STUDENTS_TEXT = """
<b>👨‍🎓 Студентам</b>

<u>Учебные материалы:</u>
Доступ к учебным материалам и расписанию занятий предоставляется через внутренний портал университета.

<u>Зачеты и экзамены:</u>
Расписание сессии публикуется на кафедре и сайте университета.

<u>Курсовые и дипломные работы:</u>
Темы курсовых и дипломных работ согласовываются с научными руководителями.
"""

SPORTS_WORK_TEXT = """
<b>⚽ Спортивная работа</b>

Кафедра активно участвует в спортивной жизни университета:

• Организация спортивных мероприятий
• Подготовка сборных команд
• Проведение соревнований
• Развитие студенческого спорта

Студенты кафедры регулярно участвуют в различных спортивных соревнованиях городского и всероссийского уровня.
"""

GTO_TESTING_CENTER_TEXT = """
<b>🏅 Центр тестирования ГТО</b>

<b>ПОДГОТОВКА К СДАЧЕ НОРМАТИВОВ ИСПЫТАНИЙ (ТЕСТОВ) ВСЕРОССИЙСКОГО ФИЗКУЛЬТУРНО-СПОРТИВНОГО КОМПЛЕКСА «ГОТОВ К ТРУДУ И ОБОРОНЕ» (ГТО)</b>

<u>📅 Сроки:</u> 
Первый квартал 2026 года (январь – март) 
по мере комплектования группы

<u>🕒 График занятий:</u> 
2 раза в неделю (понедельник, среда) с 17.00

<u>💰 Стоимость обучения:</u> 
21 500 рублей

<u>📍 Место проведения:</u> 
Санкт-Петербург, ул. Декабристов, 35.

По завершении курса предоставляется возможность сдачи нормативов ВФСК ГТО в центре тестирования ВФСК ГТО НГУ им. П.Ф. Лесгафта, Санкт-Петербург.
"""

GTO_TESTING_CENTER_TEXT_PART2 = """
<u>✅ Преимущества курса:</u>
• Профессиональная подготовка под руководством опытных тренеров и преподавателей Университета
• Индивидуальный подход к каждому участнику  
• Отработка всех обязательных и дополнительных нормативов ВФСК ГТО  
• Повышение уровня физической подготовленности  
• Для абитуриентов - дополнительные баллы (от 3-х до 10-ти) при поступлении в вузы!

<u>📞 Контакты для консультации и записи на курс:</u>

<b>Аксенова Наталья Николаевна</b>
к.п.н., доцент кафедры ТиМ МФОР / начальник центра тестирования 
ВФСК ГТО НГУ им. П.Ф. Лесгафта, Санкт-Петербург 
📧 n.aksenova@lesgaft.spb.ru 
📧 gto@lesgaft.spb.ru 
📱 +7 904 618-33-11

<b>Фоль Анастасия Сергеевна</b>
Преподаватель кафедры ТиМ МФОР 
📧 a.foll@lesgaft.spb.ru 
📱 +7 960 004-49-98
"""

STAFF_TEXT = """
<b>👨‍🏫 Сотрудники кафедры</b>

<u>Заведующий кафедрой:</u>
<b>Вишнякова Юлия Юрьевна</b> - к.п.н., доцент, зав. каф.

<u>Профессорско-преподавательский состав:</u>
• <b>Лаврухина Галина Михайловна</b> - к.п.н., доцент, доцент каф.
• <b>Горбунова Татьяна Владимировна</b> - старший преподаватель каф.
• <b>Аксенова Наталья Николаевна</b> - к.п.н., доцент каф.
• <b>Черная Анастасия Игоревна</b> - к.п.н., доцент, доцент каф.
• <b>Фигон Яана Юрьевна</b> - преподаватель каф.
• <b>Соколов Максим Сергеевич</b> - преподаватель каф.
• <b>Степин Илья Евгеньевич</b> - преподаватель каф.
• <b>Константинова Анастасия Константиновна</b> - старший преподаватель каф.
• <b>Фоль (ур. Кострикова) Анастасия Сергеевна</b> - преподаватель каф.
"""

# Функция для создания клавиатуры
def get_main_keyboard():
    keyboard = [
        ["📢 Новости", "🗓️ Расписание консультаций"],
        ["📚 История кафедры", "🎓 Абитуриентам"],
        ["👨‍🎓 Студентам", "⚽ Спортивная работа"],
        ["🏅 Центр тестирования ГТО", "👨‍🏫 Сотрудники кафедры"]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Отправляет приветственное сообщение когда пользователь отправляет /start"""
    try:
        await update.message.reply_text(WELCOME_TEXT, parse_mode='HTML', reply_markup=get_main_keyboard())
        logger.info(f"Пользователь {update.effective_user.id} запустил бота")
    except Exception as e:
        logger.error(f"Ошибка в команде start: {e}")

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обрабатывает текстовые сообщения и показывает соответствующий раздел"""
    try:
        text = update.message.text
        
        if text == "📢 Новости":
            response = NEWS_TEXT
            button_stats["📢 Новости"] += 1
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "🗓️ Расписание консультаций":
            response = CONSULTATION_SCHEDULE_TEXT
            button_stats["🗓️ Расписание консультаций"] += 1
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "📚 История кафедры":
            button_stats["📚 История кафедры"] += 1
            # Отправляем историю по частям
            await update.message.reply_text(HISTORY_TEXT_PART1, parse_mode='HTML')
            await update.message.reply_text(HISTORY_TEXT_PART2, parse_mode='HTML')
            await update.message.reply_text(HISTORY_TEXT_PART3, parse_mode='HTML')
            await update.message.reply_text(HISTORY_TEXT_PART4, parse_mode='HTML')
            await update.message.reply_text(HISTORY_TEXT_PART5, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "🎓 Абитуриентам":
            response = APPLICANTS_TEXT
            button_stats["🎓 Абитуриентам"] += 1
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "👨‍🎓 Студентам":
            response = STUDENTS_TEXT
            button_stats["👨‍🎓 Студентам"] += 1
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "⚽ Спортивная работа":
            response = SPORTS_WORK_TEXT
            button_stats["⚽ Спортивная работа"] += 1
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "🏅 Центр тестирования ГТО":
            button_stats["🏅 Центр тестирования ГТО"] += 1
            await update.message.reply_text(GTO_TESTING_CENTER_TEXT, parse_mode='HTML')
            await update.message.reply_text(GTO_TESTING_CENTER_TEXT_PART2, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        elif text == "👨‍🏫 Сотрудники кафедры":
            response = STAFF_TEXT
            button_stats["👨‍🏫 Сотрудники кафедры"] += 1
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
            
        else:
            response = "Пожалуйста, используйте кнопки меню для навигации."
            await update.message.reply_text(response, parse_mode='HTML', reply_markup=get_main_keyboard())
        
        # Сохраняем статистику после каждого нажатия
        if text in button_stats:
            save_stats(button_stats)
        
    except Exception as e:
        logger.error(f"Ошибка обработки сообщения: {e}")
        await update.message.reply_text("Произошла ошибка при обработке запроса. Попробуйте позже.", reply_markup=get_main_keyboard())

async def stat_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Показывает статистику обращений к кнопкам"""
    try:
        stat_text = "<b>📊 Статистика обращений к кнопкам:</b>\n\n"
        total = 0
        
        for button, count in button_stats.items():
            stat_text += f"• {button}: {count} раз\n"
            total += count
        
        stat_text += f"\n<b>Всего обращений:</b> {total}"
        
        await update.message.reply_text(stat_text, parse_mode='HTML')
        logger.info(f"Пользователь {update.effective_user.id} запросил статистику")
    except Exception as e:
        logger.error(f"Ошибка в команде stat: {e}")
        await update.message.reply_text("Ошибка при получении статистики")

async def statreset_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Сбрасывает статистику обращений"""
    try:
        global button_stats
        
        # Сбрасываем все счетчики
        for button in button_stats:
            button_stats[button] = 0
        
        # Сохраняем сброшенную статистику
        save_stats(button_stats)
        
        await update.message.reply_text("✅ Статистика успешно сброшена!")
        logger.info(f"Пользователь {update.effective_user.id} сбросил статистику")
    except Exception as e:
        logger.error(f"Ошибка в команде statreset: {e}")
        await update.message.reply_text("Ошибка при сбросе статистики")

async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обрабатывает ошибки"""
    logger.error(f"Ошибка при обработке обновления: {context.error}")

# Создаем Flask приложение для health checks
def create_flask_app():
    app = Flask(__name__)

    @app.route('/')
    def home():
        return "🤖 Telegram Bot для кафедры ТиМ МФОР работает!"

    @app.route('/health')
    def health():
        return "OK", 200

    @app.route('/stats')
    def stats():
        return {
            "status": "running",
            "button_stats": button_stats,
            "total_requests": sum(button_stats.values())
        }

    return app

def run_flask():
    """Запускает Flask сервер в отдельном потоке"""
    app = create_flask_app()
    port = int(os.environ.get('PORT', 10000))
    app.run(host='0.0.0.0', port=port, debug=False, use_reloader=False)

def main() -> None:
    """Запуск бота"""
    # Проверяем наличие токена
    if not BOT_TOKEN:
        logger.error("❌ BOT_TOKEN не установлен! Установите переменную окружения BOT_TOKEN в Render.")
        print("❌ ОШИБКА: BOT_TOKEN не установлен!")
        print("💡 Решение: Установите переменную окружения BOT_TOKEN в настройках Render")
        return
    
    try:
        # Запускаем Flask в отдельном потоке для health checks
        flask_thread = threading.Thread(target=run_flask, daemon=True)
        flask_thread.start()
        print("🌐 HTTP-сервер запущен для health checks")
        
        # Создаем Application для бота
        application = Application.builder().token(BOT_TOKEN).build()
        
        # Добавляем обработчики
        application.add_handler(CommandHandler("start", start))
        application.add_handler(CommandHandler("stat", stat_command))
        application.add_handler(CommandHandler("statreset", statreset_command))
        application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
        
        # Добавляем обработчик ошибок
        application.add_error_handler(error_handler)
        
        # Запускаем бота
        print("=" * 60)
        print("🤖 Бот кафедры ТиМ МФОР запущен!")
        print("📍 Название: Кафедра теории и методики массовой")
        print("            физкультурно-оздоровительной работы")
        print("🏫 НГУ им. П.Ф. Лесгафта, Санкт-Петербург")
        print("🌐 Хостинг: Render.com (Web Service)")
        print("📚 Версия: python-telegram-bot 21.7")
        print("🏥 Health checks: http://your-app.onrender.com/health")
        print("=" * 60)
        print("📊 Скрытые команды для администратора:")
        print("   /stat      - показать статистику обращений")
        print("   /statreset - сбросить статистику")
        print("=" * 60)
        print("⚡ Бот работает в режиме опроса...")
        print("=" * 60)
        
        application.run_polling()
        
    except Exception as e:
        logger.error(f"Ошибка при запуске бота: {e}")
        print(f"❌ Критическая ошибка при запуске бота: {e}")

if __name__ == "__main__":
    main()